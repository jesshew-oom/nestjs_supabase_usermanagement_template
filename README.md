...
2. **Create a Trigger Function**
   ...
   begin
     ...
     user_full_name := coalesce(new.raw_user_meta_data->>'full_name', 'default_user');
     user_email := coalesce(new.email, '');

     -- Validate required fields
     if user_email is null or user_email = '' then
       raise notice 'handle_new_user: Missing email for user_id %', new.id;
       raise exception 'Cannot create user profile: email is required' using errcode = '23502';
     end if;

     -- Attempt to insert user profile
     begin
       insert into public.temp_users (id, full_name, email)
       values (
         new.id,
         user_full_name,
         user_email
       ...
       raise notice 'handle_new_user: Successfully created profile for user_id %, email: %', 
         new.id, 
         substring(user_email from 1 for 3) || '***';
     exception
       when unique_violation then
         raise notice 'handle_new_user: User profile already exists for user_id %', new.id;
         -- Allow trigger to continue - profile already exists
       when foreign_key_violation then
         raise notice 'handle_new_user: Foreign key violation for user_id %', new.id;
         raise exception 'Cannot create user profile: invalid foreign key reference' 
           using errcode = '23503';
       when others then
         raise notice 'handle_new_user: Unexpected error for user_id %: %', new.id, sqlerrm;
...
This SQL function is a trigger function that automatically inserts a new user entry into the `public.temp_users` table when a new user signs up via Supabase Auth. It extracts the `id`, `full_name` and `email` from the `auth.users` table and inserts them into the corresponding columns in the `public.temp_users` table.

**Enhanced Error Handling:**
- **Null Safety**: Uses `coalesce` to handle missing `full_name` or `email` values
- **Validation**: Explicitly checks for required fields before insertion
- **Logging**: Uses `RAISE LOG` to log operations and errors for debugging
- **Exception Handling**: Catches and handles specific error types:
  - `unique_violation`: Handles duplicate user profiles gracefully
  - `foreign_key_violation`: Prevents invalid foreign key references
  - Generic errors: Logs and re-raises with context
- **Security**: Uses `security definer` to ensure proper permissions while maintaining security
